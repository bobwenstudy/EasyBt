...
static struct net_buf *get_rx(void)
{
    ...
    {
    case H4_EVT:
        return bt_buf_get_evt(rx.evt.evt, rx.discardable);
    case H4_ACL:
        return bt_buf_get_rx(BT_BUF_ACL_IN);
#if defined(CONFIG_BT_ISO)
    case H4_ISO:
        return bt_buf_get_rx(BT_BUF_ISO_IN);
#endif
    }
...
static inline void process_tx(void)
{
    ...
    if (!tx.buf)
    {
        tx.buf = net_buf_slist_get(&tx.fifo);
        if (!tx.buf)
        {
...
int hci_h4_init(const struct bt_hci_h4_driver *h4)
{
    ...
    sys_slist_init(&rx.fifo);
    sys_slist_init(&tx.fifo);
