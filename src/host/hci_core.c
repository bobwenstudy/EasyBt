...
void hci_tx_cmd_packet(struct net_buf *buf)
{
    bt_send(buf);
}
                         ...
                         size_t num_handlers)
{
    ...
    if (err == -EOPNOTSUPP)
    {
        LOG_WRN("Unhandled event 0x%02x len %u", event, buf->len);
    }
                            ...
                            const struct event_handler *handlers, size_t num_handlers)
{
    ...
    if (err == -EOPNOTSUPP)
    {
        LOG_WRN("Unhandled vendor-specific event");
    }
...
int bt_hci_cmd_send(uint16_t opcode, struct net_buf *buf)
{
    ...
    hci_tx_cmd_packet(buf);

    return 0;
}

int bt_hci_cmd_sync_wait(uint16_t opcode_exp, uint8_t* status, struct net_buf **rsp)
{
    struct net_buf* buf = NULL;
    uint32_t tickstart = sys_clock_tick_get();
    while (1)
    {
        if ((sys_clock_tick_get() - tickstart) > HCI_CMD_TIMEOUT)
        {
            return -1;
        }

        if (!sys_slist_is_empty(&bt_dev.rx_evt_queue)) 
        {
            break;
        }
        
        if (bt_dev.drv->polling)
        {
            bt_dev.drv->polling();
        }
    }

    buf = net_buf_slist_get(&bt_dev.rx_evt_queue);
    ...
    struct bt_hci_evt_cmd_status *evt_status;
    switch (hdr->evt)
    {
        ...
        case BT_HCI_EVT_CMD_STATUS:
            evt_status = net_buf_pull_mem(buf, sizeof(*evt_status));

            *status = evt_status->status;

            ncmd = evt_status->ncmd;
            opcode = sys_le16_to_cpu(evt_status->opcode);
            break;
        default:
            break;
    }
    
    LOG_DBG("opcode 0x%04x opcode_exp 0x%04x status %x", opcode, opcode_exp, *status);
    bt_dev.ncmd_sem += ncmd;
...
int bt_hci_cmd_send_sync(uint16_t opcode, struct net_buf *buf, struct net_buf **rsp)
{
    ...
    while(1)
    {
        if(bt_hci_cmd_sync_wait(opcode, &status, &rsp_buf))
        {
            return -1;
        }
...
static void hci_acl(struct net_buf *buf)
{
    ...
    LOG_DBG("handle %u len %u flags %u", bt_acl_handle(handle), len, flags);
    ...
    if (!conn)
    {
        LOG_ERR("Unable to find conn for handle %u", bt_acl_handle(handle));
        net_buf_unref(buf);
        return;
    }
...
static void le_legacy_conn_complete(struct net_buf *buf)
{
    ...
    if (!conn)
    {
        ...
        LOG_ERR("No pending conn for peer");
    ...
    conn->handle = handle;
    bt_addr_le_copy(&conn->le.dst, &evt->peer_addr);
    bt_addr_le_copy(&conn->le.peer, &evt->peer_addr);
...
static int hci_send_cmd_le_set_event_mask(void)
{
    ...
    if ((BT_FEAT_LE_PHY_2M(bt_dev.le.features) || BT_FEAT_LE_PHY_CODED(bt_dev.le.features)))
    {
...
static void hci_notify_rx_event(struct bt_hci_evt_hdr *hdr, struct net_buf *buf)
{
    if (bt_dev.hci_state == HCI_STATE_BOOTING_WAIT ||
        bt_dev.hci_state == HCI_STATE_PREPARING_WAIT)
    {
...
static int le_set_event_mask(void)
{
     ...
     */
#if defined(CONFIG_BT_ISO)
    if (BT_FEAT_LE_CIS(bt_dev.le.features))
    {
...
#if defined(CONFIG_BT_ISO)
    if (BT_FEAT_LE_BIS(bt_dev.le.features))
    {
#if defined(CONFIG_BT_ISO_BROADCASTER)
...
#if defined(CONFIG_BT_ISO)
    if (BT_FEAT_LE_ISO(bt_dev.le.features))
    {
        err = le_init_iso();
        if (err)
        {
...
#if defined(CONFIG_BT_CONN) && defined(CONFIG_BT_DATA_LEN_UPDATE) && defined(CONFIG_BT_AUTO_DATA_LEN_UPDATE)
    if (BT_FEAT_LE_DLE(bt_dev.le.features))
    {
...
#if defined(CONFIG_BT_SMP) && defined(CONFIG_BT_LOG_SNIFFER_INFO)
...
static void bt_init_end(int err)
{
    LOG_INF("work end. err: %d", err);
...
int bt_recv(struct net_buf *buf)
{
    ...
    {
        struct bt_hci_evt_hdr *hdr = (void *)buf->data;
        net_buf_slist_put(&bt_dev.rx_evt_queue, buf);
        return 0;
    }
#if defined(CONFIG_BT_ISO)
    case BT_BUF_ISO_IN:
...
static int hci_init_process(void)
{
    int err;
...
int hci_chip_init_state_polling(void)
{
    struct net_buf *rsp;
    int err;
...
void bt_polling_work(void)
{
    if(!bt_is_ready())
    {
        int err = hci_chip_init_state_polling();
        if(err)
        {
            bt_init_end(err);
        }
        else if(bt_dev.hci_state == HCI_STATE_INITIALING)
        {
            err = bt_init();
            bt_init_end(err);
        }
...
int bt_enable(bt_ready_cb_t cb)
{
    ...
    sys_slist_init(&bt_dev.rx_evt_queue);
...
{
    return bt_dev.hci_state == HCI_STATE_READY;
}
...
{
    if (bt_dev.hci_state == HCI_STATE_PREPARING_WAIT)
    {
        bt_dev.hci_state = HCI_STATE_INITIALING;
    }
}
